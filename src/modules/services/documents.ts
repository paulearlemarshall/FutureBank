...
import { deleteDocumentBlob, documentUploadPath, getDocumentBlob, putDocumentBlob, readAndValidateCompletedBlob } from "@/lib/document-storage";
...
function meta(row: typeof customerDocumentFiles.$inferSelect): DocumentMeta {
  return { slot: row.slot, filename: row.filename, mimeType: row.mimeType, sizeBytes: row.sizeBytes, uploadedBy: row.uploadedBy, uploadedAt: row.uploadedAt.toISOString() };
}
...
async function persistDocument(input: {
  customerId: string; customerNumber: string; slot: DocumentSlot; filename: string; mimeType: string; sizeBytes: number;
  ...
  const stored = await db.transaction(async (tx) => {
    await tx.execute(sql`select pg_advisory_xact_lock(hashtext(${`${input.customerId}:${input.slot}`}))`);
    [prior] = await tx.select().from(customerDocumentFiles).where(and(eq(customerDocumentFiles.customerId, input.customerId), eq(customerDocumentFiles.slot, input.slot))).limit(1);
    ...
    const values = {
      ...
      uploadedAt, isSeeded: input.isSeeded ?? false, updatedAt: uploadedAt,
    };
    const [row] = await tx.insert(customerDocumentFiles).values({ id: prior?.id, customerId: input.customerId, slot: input.slot, ...values })
      .onConflictDoUpdate({ target: [customerDocumentFiles.customerId, customerDocumentFiles.slot], set: values }).returning();
    await tx.insert(auditEvents).values({
      ...
      correlationId: crypto.randomUUID(), before: prior ? { slot: prior.slot, filename: prior.filename, mimeType: prior.mimeType, sizeBytes: prior.sizeBytes } : null,
      after: { slot: row.slot, filename: row.filename, mimeType: row.mimeType, sizeBytes: row.sizeBytes },
    });
    return row;
  });
...
export async function uploadCustomerDocument(input: { customerNumber: string; slot: DocumentSlot; file: File | { name: string; type: string; content: Buffer } }, actor: SessionUser) {
  ...
  try {
    return await persistDocument({ customerId: customer.id, customerNumber: customer.customerNumber, slot: input.slot, filename: validation.filename, mimeType: validation.mimeType, sizeBytes: validation.sizeBytes, sha256: createHash("sha256").update(content).digest("hex"), blob }, actor);
  } catch (error) {
...
export async function completeClientDocumentUpload(input: { customerNumber: string; slot: DocumentSlot; filename: string; blob: PutBlobResult }, actor: SessionUser) {
  ...
  return persistDocument({ customerId: customer.id, customerNumber: customer.customerNumber, slot: input.slot, filename: validation.filename, mimeType: validation.mimeType, sizeBytes: validation.sizeBytes, sha256: checked.sha256, blob: input.blob }, actor);
}
...
export async function seedDocumentRecord(input: { customerId: string; customerNumber: string; slot: DocumentSlot; filename: string; mimeType: string; sizeBytes: number; sha256: string; blob: StoredBlob }) {
  return persistDocument({ ...input, isSeeded: true, uploadedAt: new Date("2026-07-20T08:00:00.000Z") }, { id: "system.seed", username: "system.seed" });
}
