...
export async function seedBaseline(tx: SeedDb): Promise<void> {
  ...
  await tx.insert(tables.identityDocuments).values(baselineCustomers.flatMap((customer, index) => {
    ...
    return documents.map((document, documentIndex) => ({
      ...
      expiresAt: customer.customerNumber === "C000009" ? "2026-06-30" : customer.customerNumber === "C000003" && document.type === "EMIRATES_ID" ? "2026-08-10" : `2031-0${index + 1}-14`,
      ...
      expiryAlertAt: customer.customerNumber === "C000009" ? "2026-06-01" : customer.customerNumber === "C000003" && document.type === "EMIRATES_ID" ? "2026-07-20" : `2030-10-${(10 + index + documentIndex).toString().padStart(2, "0")}`,
    }));
  }));
  ...
  const caseRows = [
    ...
    { customerNumber: "C000007", reference: "KYC-000007", type: "PERIODIC_REVIEW" as const, jurisdiction: "AE", status: "IN_PROGRESS" as const, score: 20, calculated: "LOW" as const, final: null, edd: false, dueAt: "2026-08-20T17:00:00.000Z", submittedAt: null, decidedAt: null, decidedBy: null, comment: null },
    { customerNumber: "C000008", reference: "KYC-000008", type: "ONBOARDING" as const, jurisdiction: "GB", status: "PENDING_APPROVAL" as const, score: 15, calculated: "LOW" as const, final: null, edd: false, dueAt: "2026-07-22T17:00:00.000Z", submittedAt: "2026-07-20T11:00:00.000Z", decidedAt: null, decidedBy: null, comment: null },
  ...
  const evidenceRows = caseRows.flatMap((item) => {
    ...
    return types.map((evidenceType, index) => ({
      ...
      source: "Customer supplied fictional metadata", receivedAt: item.customerNumber === "C000009" ? "2024-06-10" : "2026-07-10",
      ...
      verifiedAt: ["C000003", "C000007"].includes(item.customerNumber) && evidenceType === "RESIDENCY" ? null : new Date(item.customerNumber === "C000009" ? "2024-06-11T10:00:00.000Z" : "2026-07-11T10:00:00.000Z"),
      expiresAt: item.customerNumber === "C000009" ? "2026-06-30" : item.customerNumber === "C000003" && evidenceType === "EMIRATES_ID" ? "2026-08-10" : "2028-07-10",
  ...
  const facilities = [
    ...
    { reference: "ODF-000006", accountNumber: "1000000019", requestedLimit: "40000.00", approvedLimit: "25000.00", currency: "GBP", rate: "12.2500", purpose: "Seasonal working-capital increase", riskGrade: "B", status: "PENDING_CHANGE" as const, startDate: "2025-12-01", reviewDate: "2026-12-01", expiryDate: "2027-12-01", approvedBy: staffId("supervisor") },
  ...
  await tx.insert(tables.accountHolds).values([
    { id: stableUuid("hold-pending-pep"), reference: "HLD-000001", accountId: bankAccountId("1000000004"), paymentOrderId: pendingPaymentId, amount: "2500.00", currency: "AED", status: "ACTIVE", expiresAt: new Date("2026-07-21T09:00:00.000Z") },
  ...
  const seededWorkItems = [
    ...
    { id: stableUuid("work-kyc-c8"), reference: "WRK-000004", type: "KYC_APPROVAL" as const, status: "ASSIGNED" as const, priority: "HIGH" as const, entityType: "KYC_CASE", entityReference: "KYC-000008", title: "Approve SME onboarding KYC", description: "Review ownership, evidence, risk and clear fictional screening.", requiredRole: "COMPLIANCE" as const, assignedTo: staffId("compliance"), dueAt: new Date("2026-07-22T17:00:00.000Z") },
    { id: stableUuid("work-overdraft-change"), reference: "WRK-000005", type: "OVERDRAFT_CHANGE" as const, status: "OPEN" as const, priority: "NORMAL" as const, entityType: "OVERDRAFT", entityReference: "ODF-000006", title: "Approve overdraft limit increase", description: "Review the proposed increase from GBP 25,000 to GBP 40,000.", requiredRole: "SUPERVISOR" as const, dueAt: new Date("2026-07-23T17:00:00.000Z") },
  ...
  for (const item of seededWorkItems) {
    const createdAt = new Date(item.reference === "WRK-000009" ? "2026-06-30T09:00:00.000Z" : "2026-07-18T09:00:00.000Z");
    ...
    if (item.status === "ASSIGNED") {
      ...
      seededWorkItemEvents.push({ id: stableUuid(`event-assigned-${item.reference}`), workItemId: item.id, eventType: "ASSIGNED", fromStatus: "OPEN", toStatus: "ASSIGNED", actorUserId: staffId(assigneeKey), actorUsername: `bp.${assigneeKey}`, comment: "Work item claimed for review.", occurredAt: new Date("2026-07-20T09:30:00.000Z") });
      continue;
    }
