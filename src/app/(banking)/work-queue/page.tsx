...
export default async function WorkQueuePage({ searchParams }: { searchParams: Promise<{ status?: string; type?: string; priority?: string; overdue?: string }> }) {
  ...
  return <AutomationPage name="work-queue">
    ...
    <Panel title="Queue filters"><form action="/work-queue" method="get" className="filter-bar" data-bp="work-queue-filters">
      ...
      <label htmlFor="work-filter-type">Type</label><select id="work-filter-type" name="type" data-bp="work-filter-type" defaultValue={filters.type ?? ""}><option value="">All types</option>{["KYC_APPROVAL", "PAYMENT_APPROVAL", "PAYMENT_REVERSAL", "OVERDRAFT_APPROVAL", "OVERDRAFT_CHANGE", "OVERDRAFT_ALERT", "ACCOUNTING_PERIOD_CLOSE", "GENERAL_LEDGER_JOURNAL"].map((value) => <option key={value}>{value}</option>)}</select>
