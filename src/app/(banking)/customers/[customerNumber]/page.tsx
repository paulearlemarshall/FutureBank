...
export default async function CustomerDetailPage({ params, searchParams }: { params: Promise<{ customerNumber: string }>; searchParams: Promise<{ tab?: string }> }) {
  ...
  return (
    <AutomationPage name="customer-detail">
      ...
      {activeTab === "Documents" ? <Panel title="Customer documents" description="Private customer document collection. JPEG, PNG or PDF; maximum 4 MB."><CustomerDocuments customerNumber={customer.customerNumber} documents={customer.documents} documentCollection={customer.documentCollection} uploadPrefix={documentBlobPrefix()} canEdit={Boolean(currentUser && hasPermission(currentUser.role, "KYC_GATHER"))} /></Panel> : null}
