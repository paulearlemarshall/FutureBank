...
import { getCustomerDocumentContentByReference } from "@/modules/services/documents";
...
export async function GET(_request: Request, { params }: { params: Promise<{ customerNumber: string; slot: string }> }) {
  ...
  const result = await getCustomerDocumentContentByReference(customerNumber, documentReference);
  if (!result) notFound();
