...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ query?: string }> }) {
  ...
  return <AutomationPage name="search">
    <PageHeader eyebrow="Workspace" title="Universal search" description="Search customer names, customer numbers, RIM, identity documents and account numbers." />
    ...
    <Panel title={`Customers (${customers.length})`}><div style={{ margin: "-16px" }}><table className="data-table" data-bp="global-customer-results"><thead><tr><th>Customer number</th><th>RIM</th><th>Name</th><th>Type</th><th>Status</th><th>KYC</th></tr></thead><tbody>{customers.map((customer) => <tr key={customer.customerNumber}><td className="mono"><Link href={`/customers/${customer.customerNumber}`} data-bp={`global-customer-${customer.customerNumber}`}>{customer.customerNumber}</Link></td><td className="mono">{customer.rimNumber}</td><td>{customer.displayName}</td><td>{labelEnum(customer.partyType)}</td><td><Badge tone={customer.status === "ACTIVE" ? "positive" : "warning"}>{labelEnum(customer.status)}</Badge></td><td>{labelEnum(customer.kycStatus)}</td></tr>)}</tbody></table>{query && customers.length === 0 ? <div className="empty-state">No matching customers.</div> : null}</div></Panel>
