...
import { PaymentReversalRequestForm } from "@/components/banking/payment-reversal-forms";
...
import { requestPaymentReversalAction } from "@/modules/actions/payment-reversals";
...
export default async function PaymentDetailPage({ params }: { params: Promise<{ reference: string }> }) {
  ...
  return <AutomationPage name="payment-approval-detail"><Breadcrumbs items={[{ label: "Payments", href: "/payments" }, { label: reference }]} />
    ...
    {payment.status === "BOOKED" ? <Panel title="Payment reversal" description="A full reversal requires a different supervisor and posts an equal-and-opposite ledger transaction.">{payment.reversal ? <StatusRegion id="payment-reversal-existing" tone={payment.reversal.status === "BOOKED" ? "success" : "warning"}>Reversal <Link href={`/payment-reversals/${payment.reversal.reference}`}>{payment.reversal.reference}</Link> is {labelEnum(payment.reversal.status)}.</StatusRegion> : <PaymentReversalRequestForm action={requestPaymentReversalAction} paymentReference={reference} idempotencyKey={crypto.randomUUID()} />}</Panel> : null}
  </AutomationPage>;
}
