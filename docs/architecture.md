...
| General ledger and trial balance | general-ledger policy/service, chart, journal and line tables | subledger projection coverage, exact balance checks, concurrent manual-journal decision and trial-balance API/UI journeys |
| Loan origination | loan-origination policy/service, application/detail/schedule tables and existing workflow/ledger owners | affordability and schedule policy tests, maker-checker race, exact disbursement and API/UI journeys |
...
- Every booked runtime subledger transaction synchronously owns exactly one posted general-ledger journal in the same database transaction. Account legs map to currency customer-deposit control accounts; clearing legs map to settlement, fee-income or deposit-interest-expense accounts according to transaction type. Projection failure rolls back the source booking.
- Manual general-ledger journals are exact same-currency two-line entries. A Supervisor submits into `PENDING_APPROVAL`; a distinct Admin locks and rechecks the work item, open period, accounts and balance before posting exactly once or rejecting without movement. Trial balances include `POSTED` journals only, and period close rejects pending journals, missing projections or inconsistent posted totals within the period.
- A loan application snapshots exact principal, rate, term and affordability evidence but moves no money before approval. A distinct Supervisor/Admin locks and rechecks current KYC, restrictions, product, destination, posting period and work-item version. Approval atomically creates one read-only loan account, an exact integer-minor-unit schedule, one balanced subledger transaction, one posted general-ledger projection and one destination credit; rejection creates no accounting movement.
